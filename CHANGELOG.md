# Changelog

## [0.3.9] - 2026-08-15

### 性能
- **大幅加快端点加载**：解析端点时改为按 group/id 去重后**并行**读取 API Key，并对读取结果加缓存（配置/SecretStorage 变更时自动失效）。此前同一供应商共享 Key 的多个端点会串行、重复访问系统钥匙串（Windows 上每次都是 IPC + DPAPI），导致模型选择器/请求加载明显卡顿

### 界面
- **设置面板配色改为跟随 VS Code 主题**：使用 `--vscode-*` 主题变量，深浅色自动适配，不再手写配色
- 设置面板工具栏新增「原生设置」入口：打开 VS Code 原生设置页（`@ext:llm-bridge` 过滤），方便直接编辑 JSON

## [0.3.8] - 2026-08-15

### 功能
- **恢复 Webview 设置面板**（`LLM Bridge: 设置`）：集中管理端点（增删、开关 imageInput/thinking）、视觉代理选择、API Key 导入、测试连通性、查询余额、刷新模型
- **命令收敛**：命令面板只保留「设置 / 添加模型（预设）/ 刷新模型列表」三个主命令，其余低频功能（管理模型、测试连通性、查询余额、配置视觉代理、导入 Key）收进设置面板，不再平铺命令面板
- chatView 视图标题栏新增设置入口（齿轮图标）

### 修复
- 修复「打开模型设置」命令在署名迁移后失效的问题（残留 `@ext:local.llm-bridge` 已清理；该命令现在打开设置面板）

## [0.3.7] - 2026-08-14

### 修复
- **思考强度下拉失效**：模型选择器选择的「关闭 / 高 / 最大」从未生效（请求恒发高）。根因：读取优先级把 `modelOptions` 放在 `modelConfiguration` 之前，而 VS Code 把选择器配置放在 `modelConfiguration`（`modelOptions` 永远非空且不含 `reasoningEffort`）。现改为 `modelConfiguration` 优先，与 VS Code 内置 BYOK provider 及参考实现一致
- **视觉代理不可用**（0.3.6 已知多模态代理问题）：优先枚举自己配置的 `imageInput=true` 端点，直接走 OpenAI 客户端看图，不再依赖不可靠的宿主能力上报；自己的多模态端点（如 MiMo v2.5）不再被排除在视觉代理候选之外

### 其他
- 发布署名由本地 `local` 迁移为正式 `wolfox-labs`（扩展 ID 变更，SecretStorage 中的 API key 需重新录入）

## [0.3.6] - 2026-08-06

### 功能
- **思考模式默认开启**：接入端点时不再默认关闭思考；思考强度统一由运行时下拉（关 / 高 / 最大）控制
- 端点未显式声明 `thinking` 时默认视为支持思考（`sendThinkingParam` 同理）
- 移除接入时「是否支持思考模式」的一刀切询问（避免把可思考的模型误关）

### 修复
- 修复 DeepSeek 思考模式下多轮对话 400：请求开启思考时，历史中的 assistant 消息必须回传 `reasoning_content` 字段（空串即可），否则上游报 "The reasoning_content in the thinking mode must be passed back to the API"

## [0.3.5] - 2026-08-06

### 功能
- **上报真实 token 用量给 Copilot Chat**：通过 `mimeType='usage'` 的数据 part 上报每次请求的 token 数，聊天面板右下角「会话信息」面板显示已用/总 token 与百分比，响应脚注显示 "X in, Y out"（此前面板不工作、脚注无统计）
- 模型元数据增加 `isBYOK` / `isUserSelectable`（Copilot 模型选择器按"用户自带 Key"模型处理）
- 扩展卸载/重载时主动清理模型选择器，不再残留陈旧模型条目
- 激活时先激活 Copilot Chat 再刷新模型选择器，思考强度等配置立即生效
- 多窗口同步：另一窗口增删 API Key 时本窗口模型选择器自动刷新
- 修复 `provideTokenCount` 对图片消息的 token 估算严重失真（base64 不再计入，改用固定估算）
- 接通 DeepSeek 官方价格表（`cost` 字段），VS Code「语言模型管理页」显示价格

### 其他
- 新增单元测试（usage 上报 + token 估算），共 48 例

## [0.3.0] - 2026-08-06

### 功能
- **统一端点流程重构**：移除内置模型，所有模型均来自端点配置（`llm-bridge.endpoints`）
- 所有预设流程统一：填 Key → 自动拉取 `/models` → **勾选要使用的模型（可多选）** → 设置上下文
- 新增命令「LLM Bridge: 管理模型」：按供应商分组管理，支持刷新模型列表 / 删除单个模型 / 删除分组
- 新增预设：OpenCode Zen / Go、OpenRouter
- 思考强度支持官方参数（`thinking` + `reasoning_effort`）：关闭 / 高 / 最大
- 端点配置新增 `thinking`、`sendThinkingParam`、`group` 字段；支持原生多模态模型直接解析图片

### 其他
- 模型注册 ID 改为 `custom-<id>`，发送给 API 的 `model` 名不变（与 DeepSeek V4 for Copilot 插件不冲突）

## [0.2.0] - 2026-08-03

### 安全（重要）
- API Key 改为存入系统钥匙串（`SecretStorage`），不再明文写入 `settings.json`
- 启动时自动迁移旧版 `settings.json` 中已填写的 key 到钥匙串，并清空配置项

### 功能
- **支持多个自定义 OpenAI 兼容端点**（`llm-bridge.endpoints` 数组），本地模型与云端第三方可并存
- 每个端点独立 `baseUrl` / `model` / `contextWindow` / `toolCalling` / `imageInput`，云端 Key 按端点存入钥匙串
- 命令「LLM Bridge: 添加模型（预设）」新增自定义端点引导（自动生成 id，Key 存钥匙串）

### 其他
- 迁移 `moduleResolution` 到 `node16`，消除 TypeScript 弃用警告
- 修复：自定义端点留空 API Key 时模型不显示的问题
- 新增单元测试（vitest，27 例）；`.vscodeignore` 排除测试目录

## [0.1.0] - 初始版本
